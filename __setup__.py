...
window.addEventListener("load", () => {
    ...
    body.style.setProperty("--cccs-color-r", `${randomIntFromInterval(0, 255)}`)
    body.style.setProperty("--cccs-color-g", `${randomIntFromInterval(0, 255)}`)
    body.style.setProperty("--cccs-color-b", `${randomIntFromInterval(0, 255)}`)
})
"""

SETTING_JSON = {
    "vscode_custom_css.imports": [
        path(os.path.realpath(os.path.join(".vscode", "vscodestyle.css"))),
        path(os.path.realpath(os.path.join(".vscode", "vscodejs.js"))),
    ],
...
def main():
    ...
    if not os.path.exists(os.path.join(".vscode")):
        os.mkdir(".vscode")

    zenyx.pyon.dump(SETTING_JSON, os.path.join(".vscode", "settings.json"), 4)
    
    with open(os.path.join(".vscode", "vscodestyle.css"), "w") as wf1:
        wf1.write(CSS)

    with open(os.path.join(".vscode", "vscodejs.js"), "w") as wf2:
        wf2.write(JS)
